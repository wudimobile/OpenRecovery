...
echo "还原选项：$BACKUP_NAME" > "$MENU_FILE"
echo "返回:menu:.." >> "$MENU_FILE"
echo "全部还原:shell:nandroid-restore_openrecovery.sh \"$1\" --all" >> "$MENU_FILE"
echo "还原选择项:shell:nandroid-restore_openrecovery.sh \"$1\"" >> "$MENU_FILE"
echo "全部还原(免md5):shell:nandroid-restore_openrecovery1.sh \"$1\" --all" >> "$MENU_FILE"
echo "还原选择项(免md5):shell:nandroid-restore_openrecovery1.sh \"$1\"" >> "$MENU_FILE"
...
for image in system data cache cust cdrom boot bpsw lbl logo devtree ext2; do
  ...
  case $image in
    ...
    boot)
			...
			;;
    lbl)
				echo "Bootloader:tag:nand_rest_lbl" >> "$MENU_FILE"
			;;
    logo)
				echo "Logo:tag:nand_rest_logo" >> "$MENU_FILE"
			;;
    ext2)
				echo "SD数据(ext2):tag:nand_rest_ext2" >> "$MENU_FILE"
			;;
  esac
